...
class CycleGAN3dModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        size = opt.fineSize
        depth = opt.depthSize
        self.input_A = self.Tensor(nb, opt.input_nc, depth, size, size)
        self.input_B = self.Tensor(nb, opt.output_nc, depth, size, size)
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks.GANLoss(use_lsgan=opt.no_lsgan, tensor=self.Tensor)
    ...
    def get_current_visuals(self):
        ...
        rec_A = util.tensor2im3d(self.rec_A.data)
        real_B = util.tensor2im3d(self.real_B.data)
        fake_A = util.tensor2im3d(self.fake_A.data)
        rec_B = util.tensor2im3d(self.rec_B.data)
        if self.opt.identity > 0.0:
            idt_A = util.tensor2im3d(self.idt_A.data)
            idt_B = util.tensor2im3d(self.idt_B.data)
